...
public interface DictionaryInfoDAO extends JpaRepository<DictionaryInfo,Integer> {
    ...
    @Query(nativeQuery = true, value = "select * from dictionary_info where type_id = ?1 order by sort ")
    ...
    @Query(nativeQuery = true, value = "select *from dictionary_info where value = ?1 and type_id = ?2")
    DictionaryInfo findByValueAndDictionaryType(int value, int typeId);
}
